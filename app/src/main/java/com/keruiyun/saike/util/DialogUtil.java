...
public class DialogUtil {
	...
	public static Dialog createNormalTwoDialog(Context ctx, int iconId,
			String title, String message, String btnName,
			OnClickListener listener, String btnNameTwo,
			OnClickListener listenerTwo) {
		Dialog dialog = null;
		android.app.AlertDialog.Builder builder = new android.app.AlertDialog.Builder(
				ctx);
		// 设置对话框的图标
		...
		// 添加按钮，android.content.DialogInterface.OnClickListener.OnClickListener
		builder.setPositiveButton(btnName, listener);
		builder.setNegativeButton(btnNameTwo, listenerTwo);
		builder.setCancelable(false);
		// 创建一个普通对话框
		dialog = builder.create();
		return dialog;
	...
	public static Dialog createRadioDialog(Context ctx, int iconId,
			String title, CharSequence[] items, int selectIndex,
		...
		builder.setTitle(title);
		// 0: 默认第一个单选按钮被选中
		builder.setSingleChoiceItems(items, selectIndex, listener);
		// 添加一个按钮
		builder.setPositiveButton(btnName, listener2);
		// 创建一个单选按钮对话框
		dialog = builder.create();
