...
public class ZoomView extends RelativeLayout {
    ...
    private boolean isLeftOrRightMove;
    ...
    private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
            	isLeftOrRightMove = (touchLastX - touchStartX > 0) ? true : false;
    ...
    public boolean pagerCanScroll(boolean leftOrRight)
    {
    	boolean ret = false;
    	
    	float tempLeft = 0.5f*getWidth()/zoom;
    	float tempWidth = getWidth() - tempLeft;
    	
    	if (leftOrRight)
    	{
    		if (isLeftOrRightMove)
    		{
    		    ret = tempLeft >= zoomX ? true : false;
    		}
    	}
    	else
    	{
    	    if (!isLeftOrRightMove)
    	    {
    		    ret = tempWidth <= zoomX ? true : false;
    	    }
    	}
