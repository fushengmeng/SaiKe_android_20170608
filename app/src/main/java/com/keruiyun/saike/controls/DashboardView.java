...
public class DashboardView extends View {
	...
	private int mArcQurdColor; // 字体颜色
	...
	private TitlePos mTitlePos = TitlePos.RIGHT;
	private boolean mDrawArc = false;
	private static final String TAG = "DashboardView";
	private int mArcLineColorTyle = 0;
	private int mPointerColor; // 中间颜色
	...
	public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mRadius = a.getDimensionPixelSize(R.styleable.DashboardView_radius,
				dpToPx(80));
		...
		mBigSliceCount = a.getInteger(R.styleable.DashboardView_bigSliceCount,
				10);
		mSliceCountInOneBigSlice = a.getInteger(
				R.styleable.DashboardView_sliceCountInOneBigSlice, 5);
		mArcColor = a.getColor(R.styleable.DashboardView_arcColor,
				Color.TRANSPARENT);
		mMeasureTextSize = a.getDimensionPixelSize(
				R.styleable.DashboardView_measureTextSize, spToPx(12));
		...
		mHeaderTextSize = a.getDimensionPixelSize(
				R.styleable.DashboardView_headerTextSize, spToPx(14));
		mHeaderRadius = a.getDimensionPixelSize(
				R.styleable.DashboardView_headerRadius, mRadius / 3);
		mPointerRadius = a.getDimensionPixelSize(
				R.styleable.DashboardView_pointerRadius, mRadius / 3 * 2);
		mCircleRadius = a.getDimensionPixelSize(
				R.styleable.DashboardView_circleRadius, mRadius / 17);
		mMinValue = a.getInteger(R.styleable.DashboardView_minValue, 0);
		mMaxValue = a.getInteger(R.styleable.DashboardView_maxValue, 100);
		mRealTimeValue = a.getFloat(R.styleable.DashboardView_realTimeValue,
				0.0f);
		mStripeWidth = a.getDimensionPixelSize(
				R.styleable.DashboardView_stripeWidth, 0);
		mModeType = a.getInt(R.styleable.DashboardView_stripeMode, 0);
		mBgColor = a.getColor(R.styleable.DashboardView_bgColor, 0);

		mPointerColor = Color.TRANSPARENT;
		a.recycle();
	...
	private String[] getMeasureNumbers() {
		...
		for (int i = 0; i <= mBigSliceCount; i++) {
			if (i == 0) {
				...
				// strings[i] = String
				// .valueOf(((mMaxValue - mMinValue) / mBigSliceCount) * i);
				// 安比例计算
				if (null != mStripeHighlight) {
					int degs = 0;
					for (int j = 0; j < i; j++) {
						degs += mStripeHighlight.get(j).getSweepAngle();
					}
					int val = (int) (degs / (mSweepAngle * 1.0f) * mMaxValue);
					strings[i] = String.valueOf(val);
				} else {
					strings[i] = "";
				}
			}
	...
	private void initObjects() {
		mArcQurdColor = Color.parseColor("#006ae9");
		...
		mPaintPointer.setColor(Color.parseColor("#006ae9"));
		...
		mPaintValue.getTextBounds(trimFloat(mRealTimeValue), 0,
				trimFloat(mRealTimeValue).length(), mRectRealText);
	...
	private void initSizes() {
		if (mSweepAngle > 360)
			throw new IllegalArgumentException(
					"sweepAngle must less than 360 degree");

		mSmallSliceRadius = mRadius - dpToPx(8);
		mBigSliceRadius = mSmallSliceRadius - dpToPx(-2);
		...
		if (mStartAngle <= 180 && mStartAngle + mSweepAngle >= 180) {
			mViewWidth = totalRadius * 2 + getPaddingLeft() + getPaddingRight()
					+ dpToPx(2) * 2;
		} else {
			float[] point1 = getCoordinatePoint(totalRadius, mStartAngle);
			float[] point2 = getCoordinatePoint(totalRadius, mStartAngle
					+ mSweepAngle);
		...
		if ((mStartAngle <= 90 && mStartAngle + mSweepAngle >= 90)
			...
			mViewHeight = totalRadius * 2 + getPaddingTop()
					+ getPaddingBottom() + dpToPx(2) * 2;
		} else {
			float[] point1 = getCoordinatePoint(totalRadius, mStartAngle);
			float[] point2 = getCoordinatePoint(totalRadius, mStartAngle
					+ mSweepAngle);
		...
		mRectArc = new RectF(mCenterX - mRadius, mCenterY - mRadius, mCenterX
		...
		if (mStripeWidth > 0) {
			...
			mRectStripe = new RectF(mCenterX - r, mCenterY - r, mCenterX + r,
					mCenterY + r);
		}
		...
		if (heightMode == MeasureSpec.EXACTLY) {
			...
			if (mStartAngle >= 180 && mStartAngle + mSweepAngle <= 360) {
				mViewHeight = totalRadius + mCircleRadius + dpToPx(2)
						+ dpToPx(25) + getPaddingTop() + getPaddingBottom()
						+ mRectRealText.height();
			} else {
				float[] point1 = getCoordinatePoint(totalRadius, mStartAngle);
				float[] point2 = getCoordinatePoint(totalRadius, mStartAngle
						+ mSweepAngle);
				float maxY = Math.max(Math.abs(point1[1]) - mCenterY,
						Math.abs(point2[1]) - mCenterY);
				float f = mCircleRadius + dpToPx(2) + dpToPx(25)
						+ mRectRealText.height();
				float max = Math.max(maxY, f);
				mViewHeight = (int) (max + totalRadius + getPaddingTop()
						+ getPaddingBottom() + dpToPx(2) * 2);
			}
			if (widthMode == MeasureSpec.AT_MOST)
	...
	protected void onDraw(Canvas canvas) {
		...
		drawQuadrilateralPointer(canvas);
		drawPointer(canvas);
	}
	...
	private void drawStripe(Canvas canvas) {
		if (mStripeMode != StripeMode.NORMAL && mStripeHighlight != null) {
			for (int i = 0; i < mStripeHighlight.size(); i++) {
				...
				if (highlightCR.getColor() == 0
						|| highlightCR.getSweepAngle() == 0)
					continue;

				mPaintStripe.setColor(highlightCR.getColor());
				if (highlightCR.getStartAngle() + highlightCR.getSweepAngle() <= mStartAngle
						+ mSweepAngle) {
					...
					canvas.drawArc(
							mRectStripe,
							highlightCR.getStartAngle(),
							mStartAngle + mSweepAngle
									- highlightCR.getStartAngle(), false,
							mPaintStripe);
					break;
				}
			}
	...
	private void drawMeasures(Canvas canvas) {
		...
		for (int i = 0; i <= mBigSliceCount; i++) {
			...
			if (i == 0 || i == mBigSliceCount) {
				angle = i * mBigSliceAngle + mStartAngle;
			} else {
				if (null != mStripeHighlight) {
					angle += mStartAngle;
					for (int j = 0; j <= i - 1; j++) {
						angle += mStripeHighlight.get(j).getSweepAngle();
					}
				} else {
					angle = i * mBigSliceAngle + mStartAngle;
				}
			...
			if (mStripeMode == StripeMode.NORMAL && mStripeHighlight != null) {
				for (int j = 0; j < mStripeHighlight.size(); j++) {
					...
					if (angle <= highlightCR.getStartAngle()
							+ highlightCR.getSweepAngle()) {
				...
				if (mArcLineColorTyle == 0
						&& (mBigSliceCount == mStripeHighlight.size())) {
					if (i == 0) {
						mPaintArc.setColor(mStripeHighlight.get(i).getColor());
					} else {
						mPaintArc.setColor(mStripeHighlight.get(i - 1)
								.getColor());
					}
				} else if (mArcLineColorTyle == 1
						&& (mBigSliceCount == mStripeHighlight.size())) {
					mPaintArc.setColor(mStripeHighlight.get(0).getColor());
				} else {
					mPaintArc.setColor(mArcColor);
				}
			}
			...
			if (mArcLineColorTyle == 1) {
				if (i != 0 && i != mBigSliceCount) {
					canvas.drawText(number, numberPoint[0], numberPoint[1]
							+ mRectMeasures.height(), mPaintText);
				}
			} else {
				canvas.drawText(number, numberPoint[0], numberPoint[1]
						+ mRectMeasures.height(), mPaintText);
			...
			// if (i == 0 || i == mBigSliceCount) {
			// canvas.drawText(number, numberPoint[0], numberPoint[1]
			// + (mRectMeasures.height() / 2), mPaintText);
			// } else {
			// canvas.drawText(number, numberPoint[0], numberPoint[1]
			// + mRectMeasures.height(), mPaintText);
			// }
		...
		if (mStripeMode == StripeMode.NORMAL) {
			if (mStripeHighlight != null) {
				for (int i = 0; i < mStripeHighlight.size(); i++) {
					...
					mPaintArc.setColor(highlightCR.getColor());
					if (highlightCR.getStartAngle()
							+ highlightCR.getSweepAngle() <= mStartAngle
							+ mSweepAngle) {
	...
	private void drawCircleAndReadingText(Canvas canvas) {
		...
		mPaintText.setTextAlign(Paint.Align.CENTER);
		mPaintText.getTextBounds(mHeaderTitle, 0, mHeaderTitle.length(),
				mRectHeader);

		// canvas.drawText(mHeaderTitle, mCenterX, mCenterY - mHeaderRadius
		// + mRectHeader.height(), mPaintText);

		canvas.drawText(mHeaderTitle, mCenterX, mCenterY + mCircleRadius
				+ dpToPx(2) + dpToPx(25), mPaintText);
		...
		if (mDrawArc) {
			...
			canvas.drawCircle(mCenterX, mCenterY, mCircleRadius + dpToPx(2),
					mPaintPointer);
		}

		// 绘制读数
		// canvas.drawText(trimFloat(mRealTimeValue), mCenterX, mCenterY
		// + mCircleRadius + dpToPx(2) + dpToPx(25), mPaintValue);
	}
	...
	private void drawPointer(Canvas canvas) {
		...
		canvas.drawCircle((point1[0] + point2[0]) / 2,
				(point1[1] + point2[1]) / 2, mCircleRadius / 2, mPaintPointer);
	}

	/**
	 * 绘制四边形指针
	 */
	private void drawQuadrilateralPointer(Canvas canvas) {
		mPaintPointer.setStyle(Paint.Style.FILL);
		mPaintPointer.setColor(mArcQurdColor);
		path.reset();

		Log.e(TAG, "initAngle:" + String.valueOf(initAngle));

		Log.e(TAG, "mCenterX:" + String.valueOf(mCenterX));
		Log.e(TAG, "mCenterY:" + String.valueOf(mCenterY));
		float[] point0 = getCoordinatePoint(mCircleRadius * 3 / 4,
				initAngle + 90);
		path.moveTo(point0[0], point0[1]);

		// canvas.drawCircle(point0[0], point0[1], 5, mPaintPointer);

		float[] point1 = getCoordinatePoint(mCircleRadius, initAngle - 180);

		path.lineTo(point1[0], point1[1]);

		// canvas.drawCircle(point1[0], point1[1], 5, mPaintPointer);

		Log.e(TAG, "point1X:" + String.valueOf(point0[0]));
		Log.e(TAG, "point1Y:" + String.valueOf(point0[1]));
		float[] point2 = getCoordinatePoint(mCircleRadius * 3 / 4,
				initAngle - 90);
		path.lineTo(point2[0], point2[1]);

		// canvas.drawCircle(point2[0], point2[1], 5, mPaintPointer);

		Log.e(TAG, "point2X:" + String.valueOf(point2[0]));
		Log.e(TAG, "point2Y:" + String.valueOf(point2[1]));
		float[] point3 = getCoordinatePoint(mPointerRadius, initAngle);
		path.lineTo(point3[0], point3[1]);

		// canvas.drawCircle(point3[0], point3[1], 5, mPaintPointer);

		Log.e(TAG, "point3X:" + String.valueOf(point3[0]));
		Log.e(TAG, "point3Y:" + String.valueOf(point3[1]));
		path.close();
		canvas.drawPath(path, mPaintPointer);
	}
	...
	private float getAngleFromResult(float result) {
		if (result > mMaxValue)
			return mMaxValue;
		return mSweepAngle * (result - mMinValue) / (mMaxValue - mMinValue)
				+ mStartAngle;
	}
	...
	public void setArcQurdColor(int arcQurdColor) {
		mArcQurdColor = arcQurdColor;
		mPaintPointer.setColor(mArcQurdColor);
		invalidate();
	}
	...
	public void setStripeHighlightColorAndRange(
			List<HighlightCR> stripeHighlight) {
	...
	public void setArcLineColorType(int mArcLineColorTyle) {
		this.mArcLineColorTyle = mArcLineColorTyle;
		invalidate();
	}
