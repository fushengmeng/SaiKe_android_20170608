 ...
 * to 0 = never fade out)</li>
 * <li>
 * radius: Define the circle outer radius (default to 4.0)</li>
 * <li>
 * spacing: Define the circle spacing (default to 4.0)</li>
 * <li>
 * snap: If true, the 'active' indicator snaps from one page to the next;
 * otherwise, it moves smoothly.</li>
 * </ul>
 */
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		int activeType = a.getInt(
				R.styleable.CircleFlowIndicator_CircleFlowIndicator_activeType,
				STYLE_FILL);
		...
		int activeColor = a
				.getColor(
						R.styleable.CircleFlowIndicator_CircleFlowIndicator_activeColor,
						activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a
				.getInt(R.styleable.CircleFlowIndicator_CircleFlowIndicator_inactiveType,
						STYLE_STROKE);
		...
		int inactiveColor = a
				.getColor(
						R.styleable.CircleFlowIndicator_CircleFlowIndicator_inactiveColor,
						inactiveDefaultColor);

		// Retrieve the radius
		mRadius = a.getDimension(
				R.styleable.CircleFlowIndicator_CircleFlowIndicator_radius,
				4.0f);
		...
		spacing = a.getDimension(
				R.styleable.CircleFlowIndicator_CircleFlowIndicator_spacing,
				4.0f);
		// We want the spacing to be center-to-center
		spacing += 2 * mRadiusActive;

		// Retrieve the fade out time
		fadeOutTime = a.getInt(
				R.styleable.CircleFlowIndicator_CircleFlowIndicator_fadeOut, 0);

		mCentered = a.getBoolean(
				R.styleable.CircleFlowIndicator_CircleFlowIndicator_centered,
				false);

		mSnap = a
				.getBoolean(
						R.styleable.CircleFlowIndicator_CircleFlowIndicator_snap,
						false);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			canvas.drawCircle(leftPadding + mRadius + (iLoop * spacing)
					+ centeringOffset, getPaddingTop() + mRadius,
		...
		if (mSnap) {
			...
			// else, the flow width hasn't been updated yet. Draw the default
			// position.
		}

		canvas.drawCircle(leftPadding + mRadius + cx + centeringOffset,
				getPaddingTop() + mRadius, mRadiusActive, mPaintActive);
	}
