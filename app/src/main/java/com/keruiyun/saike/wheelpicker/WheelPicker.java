...
public class WheelPicker extends View implements Tintable,IDebug, IWheelPicker, Runnable {
	...
	/**
	 * 滚轮选择器将会绘制的Item数量的一半 该值主要用于偏移值的计算，该值由滚轮选择器计算赋值
	 */
	...
	/**
	 * 滚轮选择器中心坐标 滚轮选择器的中心坐标为滚轮正中心坐标，其X坐标取值为滚轮宽度的一半，而Y坐标取值为滚轮高度的一半
	 */
	 ...
	 * 背景渐变色
	 */
	private int mColorGradientStart, mColorGradientEnd;
	...
	public WheelPicker(Context context, AttributeSet attrs) {
		...
		mData = Arrays.asList(getResources().getStringArray(
				idData == 0 ? R.array.WheelArrayDefault : idData));
		mItemTextSize = a.getDimensionPixelSize(
				R.styleable.WheelPicker_wheel_item_text_size, getResources()
						.getDimensionPixelSize(R.dimen.WheelItemTextSize));
		mVisibleItemCount = a.getInt(
				R.styleable.WheelPicker_wheel_visible_item_count, 5);
		mCurrentItemPosition = a.getInt(
				R.styleable.WheelPicker_wheel_current_item_position, 3);
		hasSameWidth = a.getBoolean(R.styleable.WheelPicker_wheel_same_width,
				false);
		mTextMaxWidthPosition = a.getInt(
				R.styleable.WheelPicker_wheel_maximum_width_text_position, 3);
		mMaxWidthText = a
				.getString(R.styleable.WheelPicker_wheel_maximum_width_text);

		mColorGradientStart = a.getColor(
				R.styleable.WheelPicker_wheel_bg_color_gradient_start, Color.BLACK);

		mColorGradientEnd = a.getColor(
				R.styleable.WheelPicker_wheel_bg_color_gradient_end, Color.parseColor("#1a1a1a"));

		mItemTextColor = a.getColor(
				R.styleable.WheelPicker_wheel_item_text_color, 0xFFAB5D2C);
		Log.e(TAG, "mItemTextColor" + String.valueOf(mItemTextColor));

		mCurrentItemTextColor =a.getColor(
				R.styleable.WheelPicker_wheel_current_item_text_color, 0xFFAB5D2C);
		Log.e(TAG,
				"mCurrentItemTextColor:init:" + String.valueOf(mCurrentItemTextColor));

		mItemSpace = a.getDimensionPixelSize(
				R.styleable.WheelPicker_wheel_item_space, getResources()
						.getDimensionPixelSize(R.dimen.WheelItemSpace));
		hasIndicator = a.getBoolean(R.styleable.WheelPicker_wheel_indicator,
				false);
		mIndicatorColor = a.getColor(
				R.styleable.WheelPicker_wheel_indicator_color, 0x00000000);
		mIndicatorSize = a.getDimensionPixelSize(
				R.styleable.WheelPicker_wheel_indicator_size, getResources()
						.getDimensionPixelSize(R.dimen.WheelIndicatorSize));
		hasCurtain = a.getBoolean(R.styleable.WheelPicker_wheel_curtain, false);
		mCurtainColor = a.getColor(R.styleable.WheelPicker_wheel_curtain_color,
				0x00000000);
	...
	private void computeTextSize() {
		if (hasSameWidth) {
			mTextMaxWidth = (int) mPaint.measureText(String.valueOf(mData
					.get(0)));
		...
		if (isDebug)
			Log.i(TAG, "Text's max size is: (" + mTextMaxWidth + ","
					+ mTextMaxHeight + ")");
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int resultHeight = mTextMaxHeight * mVisibleItemCount + mItemSpace
				* (mVisibleItemCount - 1);
	...
	protected void onSizeChanged(int w, int h, int oldW, int oldH) {
		...
		mDrawnCenterY = (int) (mWheelCenterY - ((mPaint.ascent() + mPaint
				.descent()) / 2));
	...
	private void computeFlingLimitY() {
		...
		mMinFlingY = isCyclic ? Integer.MIN_VALUE : -mItemHeight
				* (mData.size() - 1) + currentItemOffset;
	...
	private void computeIndicatorRect() {
		...
		mRectIndicatorHead.set(0, indicatorHeadCenterY - halfIndicatorSize,
				getWidth(), indicatorHeadCenterY + halfIndicatorSize);
		mRectIndicatorFoot.set(0, indicatorFootCenterY - halfIndicatorSize,
				getWidth(), indicatorFootCenterY + halfIndicatorSize);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int drawnDataPos = drawnDataStartPos + mCurrentItemPosition, drawnOffsetPos = -mHalfDrawnItemCount; drawnDataPos < drawnDataStartPos
				+ mCurrentItemPosition + mDrawnItemCount; drawnDataPos++, drawnOffsetPos++) {
			String data = "";
			if (isCyclic) {
				int actualPos = drawnDataPos % mData.size();
				actualPos = actualPos < 0 ? (actualPos + mData.size())
						: actualPos;
			...
			mPaint.setStyle(Paint.Style.FILL);
			int mDrawnItemCenterY = mDrawnCenterY
					+ (drawnOffsetPos * mItemHeight) + mScrollOffsetY
					% mItemHeight;
			if (isDebug)
				Log.i(TAG, "Item " + drawnOffsetPos + "'s draw centerY is"
						+ mDrawnItemCenterY);
			if (hasAtmospheric) {
				int alpha = (int) ((mDrawnCenterY - Math.abs(mDrawnCenterY
						- mDrawnItemCenterY))
						* 1.0F / mDrawnCenterY * 255);
				if (isDebug)
					Log.i(TAG, "Item " + drawnOffsetPos + "'s draw alpha is"
							+ alpha);
	...
	private void drawBg(Canvas canvas){
		...
		// 第1个点
		colors[0] = mColorGradientStart;
		positions[0] = 0;

		// 第2个点
		colors[1] = mColorGradientEnd;
		positions[1] = 0.25f;

		// 第3个点
		colors[2] = mColorGradientEnd;
		positions[2] = 0.75f;

		// 第4个点
		colors[3] = mColorGradientStart;
		positions[3] = 1;
		...
		canvas.drawRect(clientRect, mPaintBg);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			boolean isClick=clickDownX==upX&&clickDownY==upY;

			if (isClick){
				if (upX>mRectCurrentItem.left&&upX<mRectCurrentItem.right
						&&upY>mRectCurrentItem.top&&upY<mRectCurrentItem.bottom){
					if (null != mOnItemSelectListener){
						int position=getPosition();
						mOnItemSelectListener.onItemClick(this, mData.get(position),
								position);
					}
			...
			if (Math.abs(velocity) > mMinimumVelocity) {
				mScroller.fling(0, mScrollOffsetY, 0, velocity, 0, 0,
						mMinFlingY, mMaxFlingY);
				mScroller.setFinalY(mScroller.getFinalY()
						+ computeDistanceToEndPoint(mScroller.getFinalY()
								% mItemHeight));
			} else {
				mScroller
						.startScroll(0, mScrollOffsetY, 0,
								computeDistanceToEndPoint(mScrollOffsetY
										% mItemHeight));
			}
			// 校正坐标
			if (!isCyclic)
	...
	public void run() {
		if (mScroller.isFinished()) {
			int position = getPosition();
			if (isDebug)
				Log.i(TAG, position + ":" + mData.get(position) + ":"
						+ mScrollOffsetY);
			soundPlayer.playerSlide();
			if (null != mOnItemSelectListener)
				mOnItemSelectListener.onItemSelected(this, mData.get(position),
						position);
	...
	private int getPosition(){
		int position = (-mScrollOffsetY / mItemHeight + mCurrentItemPosition)
				% mData.size();
	...
	public void setCurrentItem(int position) {
		if (!isPosInRang(position))
			throw new ArrayIndexOutOfBoundsException(
					"Current item position must in [0, " + mData.size()
							+ "), but current is " + position);
		mCurrentItemPosition = position;
		Log.e("mScrollOffsetY", String.valueOf(mScrollOffsetY));
		Log.e("mLastPointY", String.valueOf(mLastPointY));
	...
	public void setData(List data) {
		if (null == data)
			throw new NullPointerException(
					"WheelPicker's data can not be null!");
		mData = data;
		computeTextSize();
		requestLayout();
		invalidate();
	...
	public void setMaximumWidthText(String text) {
		if (null == text)
			throw new NullPointerException(
					"Maximum width text can not be null!");
		mMaxWidthText = text;
		computeTextSize();
		requestLayout();
		invalidate();
	...
	public void setMaximumWidthTextPosition(int position) {
		if (!isPosInRang(position))
			throw new ArrayIndexOutOfBoundsException(
					"Maximum width text Position must in [0, " + mData.size()
							+ "), but current is " + position);
	...
	public interface OnItemSelectListener {
		/**
		 * 当滚轮选择器Item被选中时 滚动选择器滚动停止后会回调该方法并将当前在滚轮中心显示的数据和数据在数据列表中对应的位置返回
		 * 该方法在滚动初始化设置数据后也会调用
		 * 
		 * @param picker
		 *            滚轮选择器
		 ...
		 *            当前位于滚轮中心显示的数据
		 * @param position
		 *            当前位于滚轮中心显示的数据在数据列表中的位置
		 */
		void onItemSelected(WheelPicker picker, Object data, int position);

		void onItemClick(WheelPicker picker, Object data, int position);
	}

	/**
	 * 滚轮选择器滚动时监听接口
	 * 
	 * @author AigeStudio 新项目结构
	 * @since 2016-06-17
	 */
	public interface OnWheelChangeListener {
		 ...
		 * 
		 * @param state
		 *            滚轮选择器滚动状态，其值仅可能为下列之一 {@link WheelPicker#SCROLL_STATE_IDLE}
